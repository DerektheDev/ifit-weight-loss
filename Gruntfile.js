...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    site: {
      ...
      dest: 'dist',
      temp: '.tmp',
      ftpRoot: '/sample-ftp-deploy'
    },

    'ftp-deploy': {
      build: {
        auth: {
          host: 'dev.perficientxd.com',
          port: 21,
          authKey: 'dev'
        },
        src: '<%= site.dest%>/',
  ...
  grunt.registerTask('ftpdev', ['ftp-deploy']);

  grunt.registerTask('b',   ['build']);
  grunt.registerTask('bf',  ['build', 'ftp-deploy']);
  grunt.registerTask('bfs', ['build', 'ftp-deploy', 'serve']);
